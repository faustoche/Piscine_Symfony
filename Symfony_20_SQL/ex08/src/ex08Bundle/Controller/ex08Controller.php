...
class ex08Controller extends AbstractController {
	...
	#[Route('/ex08', name:'ex08_controller_index')]
	public function indexAction(Connection $connection, Request $request): Response {
		...
		if ($form->isSubmitted() && $form->isValid()) {
			...
			try {
				...
				return $this->redirectToRoute('ex08_controller_index');
			} catch (\Exception $e) {
	...
	// ajout d'une colonne
	#[Route('/ex08/add-column', name:'ex08_add_column')]
	...
	#[Route('ex08/update/{id}', name:'ex08_update', methods: ['GET', 'POST'])]
	public function updateAction(Connection $connection, Request $request, int $id): Response {
		...
		return $this->render('ex08/update.html.twig', [
			'form' => $form->createView()
		]);
	}
}